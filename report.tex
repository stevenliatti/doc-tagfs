...
\begin{itemize}
    \item Un tag est ajouté à un fichier :
        \begin{itemize}
            \item Index 1 : obtenir l'ensemble des fichiers associés au tag et ajouter le fichier. 
                Si tag non présent, ajouter la nouvelle clé => O(1)
            \item Index 2.1 : obtenir l'ensemble des tags d'un fichier et ajouter le tag => O(1)
            \item Index 2.2 : parcourir l'arbre à la recherche du fichier et ajouter le nouveau tag => ~ O(log(n))
        \end{itemize}
    \item Un tag est supprimé d'un fichier :
        \begin{itemize}
            \item Index 1 : obtenir l'ensemble des fichiers associés au tag et supprimer le fichier => O(1)
            \item Index 2.1 : obtenir l'ensemble des tags associés au fichier et supprimer le tag => O(1)
            \item Index 2.2 : parcourir l'arbre à la recherche du fichier et supprimer le tag => ~ O(log(n))
        \end{itemize}
    \item Un tag change de nom (opération lourde) :
        2 possibilités :
        \begin{enumerate}
            \item Dans les 2 index, les tags pointent vers une "entité" tag
                \begin{itemize}
                    \item Index 1 : "rien de spécial"
                    \item Index 2.1 et 2.2 : "rien de spécial"
                \end{itemize} 
            \item Dans les 2 index, les tags sont des valeurs (strings) inscrits en dur
                \begin{itemize}
                    \item Index 1 : renommer la clé / retirer l'ensemble et le rajouter selon le nouveau nom
                    \item Index 2.1 et 2.2 : récupérer les fichiers grâce à index 1 et boucler pour changer le nom
                \end{itemize} 
        \end{enumerate}
        Dans les deux cas, parcourir les fichiers sur le disque pour changer la valeur des tags dans les \acrshort{xattr}
    \item Un fichier est supprimé :
        \begin{itemize}
            \item Index 1 : supprimer toutes les clés correspondant aux tags
            \item Index 2.1 : supprimer la clé fichier
            \item Index 2.2 : parcourir et supprimer la clé fichier
        \end{itemize}
    \item Un fichier est copié :
        \begin{itemize}
            \item Index 1 : parcourir tous les tags du fichier et ajouter le nouveau fichier créé aux ensembles
            \item Index 2.1 et 2.2 : créer une nouvelle clé et copier l'ensemble de tags vers cette clé
        \end{itemize}
    \item Un fichier est déplacé ou change de nom :
        \begin{itemize}
            \item Index 1 : parcourir tous les tags du fichier et changer le nom du fichier
            \item Index 2.1 et 2.2 : renommer la clé / retirer l'ensemble et le rajouter selon le nouveau nom
        \end{itemize}
\end{itemize}
